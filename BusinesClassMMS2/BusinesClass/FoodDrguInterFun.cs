    ...
    public class FoodDrugInterFun
    {
        public static List<GenericDrugDetail> ViewGenDtl(int GID, string GName)
        {
            ...
            try
            {
                ...
                + " from FOODINTERACTION_VW a,L_DrugFoodInteraction b"
                + " Where a.id = b.interactingFoodId And b.genericId =" + GID + " order by a.name ";
        ...
        public static MessageModel Save(List<GenericDrugDetail> tbl, User Operator)
        {
            ...
            try
            {
                ...
                using (Con)
                {
                    ...
                    if (GenericID > 0)
                    {
                        string StrSql = "Insert into canDrugfoodInteraction (GenericID,InteractingFoodId,Discription,grouporgeneric,StartDatetime, "
                            + " EndDateTime,OperatorId, DeletedBy,DeletedDatetime) "
                            + "  select  GenericID,InteractingFoodId,Discription,grouporgeneric,StartDatetime,EndDateTime,OperatorId, " + gSavedById
                            + " , getdate()  from L_DrugFoodInteraction where genericid=" + GenericID + " ";
                        bool Del = MainFunction.SSqlExcuite(StrSql, Trans);

                        StrSql = "Delete from L_DrugFoodInteraction where genericid=" + GenericID;
                        bool DelActual = MainFunction.SSqlExcuite(StrSql, Trans);

                        foreach (GenericDrugDetail row in tbl)
                        {
                            var sql = "Insert into L_DrugFoodInteraction(genericId,InteractingFoodId,Discription)"
